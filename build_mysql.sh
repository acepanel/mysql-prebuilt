...
cmake -G Ninja .. -DCMAKE_C_COMPILER=gcc -DCMAKE_CXX_COMPILER=g++ -DCMAKE_INSTALL_PREFIX=${mysql_path} -DMYSQL_DATADIR=${mysql_path}/data -DSYSCONFDIR=${mysql_path}/conf -DWITH_MYISAM_STORAGE_ENGINE=1 -DWITH_INNOBASE_STORAGE_ENGINE=1 -DWITH_ARCHIVE_STORAGE_ENGINE=0 -DWITH_EXAMPLE_STORAGE_ENGINE=0 -DWITH_FEDERATED_STORAGE_ENGINE=0 -DWITH_BLACKHOLE_STORAGE_ENGINE=0 -DWITH_PARTITION_STORAGE_ENGINE=0 -DWITH_NDBCLUSTER_STORAGE_ENGINE=0 ${WITHOUT_TOKUDB} -DWITH_ROCKSDB=0 -DDEFAULT_CHARSET=utf8mb4 -DDEFAULT_COLLATION=utf8mb4_general_ci -DMAX_INDEXES=255 -DWITH_RAPID=0 -DWITH_NDBMTD=0 -DENABLED_LOCAL_INFILE=1 -DWITH_COREDUMPER=0 -DWITH_BUILD_ID=0 -DWITH_DEBUG=0 -DWITH_UNIT_TESTS=OFF -DINSTALL_MYSQLTESTDIR= -DCMAKE_BUILD_TYPE=Release -DWITH_SYSTEM_LIBS=1 -DWITH_RAPIDJSON=system -DWITH_SYSTEMD=1 -DSYSTEMD_PID_DIR=${mysql_path} ${WITH_BOOST} ${WITH_OPT} ${WITHOUT_EMBEDDED}
if [ "$?" != "0" ]; then
    rm -rf ${mysql_path}
